...
export async function runSpell(spellrun: SpellRunner, content: string, agent: Agent, app: Application): Promise<any> {
...
export async function createTasks(objective: string, task_description: string, result: string, incomplete_tasks: string, agent: Agent, app: Application): Promise<Array<string>> {
    const content = `{
        "objective": "${objective}",
        "result": "${result}",
...
export function findSimilarSentences(data: Data[], query: string, k: number): Data[] {
