...
export const sendSlackImage = makeFlowNodeDefinition({
  ...
  triggered: async ({ commit, read, graph: { getDependency } }) => {
    ...
    // const images = read('images') as string[] // Array of Base64 strings
    ...
    try {
      for (const base64Image of images) {
        ...
        await slack.getClient().client.files.uploadV2({
          channels: channel,
          file: stream,
          filename: 'image.png',
        })
      ...
      console.log('Error sending image to Slack:', e)
    }

    slackActionService?.sendMessage(event, { content: 'Image(s) sent.' })
