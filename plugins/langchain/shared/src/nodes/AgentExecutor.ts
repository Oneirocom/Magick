 ...
 */
const info =
  'Agent Executor is used to formulate an thought about the tools available to the agent and plan an action'
...
export class AgentExecutor extends MagickComponent<Promise<void>> {
  ...
  builder(node: MagickNode) {
    ...
    node.inspector.add(nameControl).add(inputGenerator).add(outputGenerator)

    return node.addInput(dataInput).addOutput(dataOutput)
  }
