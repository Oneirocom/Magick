...
describe('Agent Standalone Integration', () => {
  ...
  it('should load a spell into the spellbook', async () => {
    await agent.spellbook.loadSpells([
      {
  ...
  it('should emit and receive a message event', done => {
    // Setup listener for the response message
    agent.on('messageReceived', async response => {
      try {
        console.log('response', response)
        expect(response).toHaveProperty('content', 'Hello World')
        expect(response).toHaveProperty('agentId', AGENT_ID)
        expect(response).toHaveProperty('projectId', PROJECT_ID)
        done()
      } catch (error) {
        console.error(error)
        done(error)
