...
/** Information related to the TextToAudio */
const info = 'Generate audio using any of the providers available in Magick.'
...
/**
 * TextToAudio component responsible for generating text using any providers
 * available in Magick.
 */
export class TextToAudio extends MagickComponent<Promise<WorkerReturn>> {
  constructor() {
    super(
      'TextToAudio',
      {
  ...
  /**
   * Builder for generating audio from text.
  ...
  builder(node: MagickNode) {
    ...
    // get completion providers audio categories
  ...
  async worker(
    ...
    // get completion providers for audio category
    const completionProviders = pluginManager.getCompletionProviders('audio', [
      'text2audio',
    ]) as CompletionProvider[]
    ...
    const provider = completionProviders.find(provider =>
      provider.models.includes('riffusion')
    ) as CompletionProvider
