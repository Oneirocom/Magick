...
/** Information related to the ImageToText */
...
/**
 * ImageToText component responsible for generating text using any providers
 * available in Magick.
 */
export class ImageToText extends MagickComponent<Promise<WorkerReturn>> {
  constructor() {
    super(
      'Image to Text',
      {
  ...
  async worker(
    ...
    const provider = completionProviders.find(provider =>
      provider.models.includes('carot')
    ) as CompletionProvider
