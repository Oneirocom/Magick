...
/**
 * Update component responsible for updateing records using any providers
 * available in Magick.
 */
export class Update extends MagickComponent<Promise<WorkerReturn>> {
  constructor() {
    super(
      'Update Record',
      {
  ...
  builder(node: MagickNode) {
    ...
    /**
     * Configure the provided node according to the updateed database provider.
     */
    const configureNode = () => {
  ...
  /**
   * Worker for processing update queries.
  ...
  async worker(
    ...
    // get the provider for the updateed model
    const provider = completionProviders.find(provider =>
      provider.models.includes(model)
    ) as CompletionProvider
