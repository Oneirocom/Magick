...
import type { SpellRunnerService } from './services/spell-runner/spell-runner';
...
export type RequestClientService = Pick<RequestService<Params<RequestQuery>>, typeof requestServiceMethods[number]>;
...
export const createClient = <Configuration = any>(connection: TransportConnection<ServiceTypes>) => {
  ...
  client.use('agents', connection.service('agents'), { methods: agentServiceMethods });
  client.use('spells', connection.service('spells'), { methods: spellServiceMethods });
  client.use('spell-runner', connection.service('spell-runner'), { methods: spellRunnerServiceMethods });
  client.use('request', connection.service('request'), { methods: requestServiceMethods });
