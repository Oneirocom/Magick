...
describe('Editor', () => {
  ...
  it('component register', () => {
    ...
    assert.throws(
      () => Boolean(editor.getComponent('Number')),
  ...
  describe('manage', () => {
    ...
    it('connections', async () => {
      ...
      assert.strictEqual(
        (node1.outputs.get('num') as Output).connections.length,
        1,
        'one connection'
      )
    ...
    it('nodes', async () => {
      ...
      assert.throws(
        () => editor.selectNode(node2),
        'Unable to select not added node'
      )

      editor.addNode(node2)
      editor.selectNode(node2, false)
      assert.strictEqual(
        editor.selected.contains(node1),
        false,
        'Previous node unselected'
      )
      assert.strictEqual(
        editor.selected.contains(node2),
        true,
        'New node selected'
      )
    ...
    describe('prevent', () => {
      ...
      it('connection', () => {
        ...
        assert.strictEqual(
          (node.outputs.get('num') as Output).hasConnection(),
          false
        )
      })
