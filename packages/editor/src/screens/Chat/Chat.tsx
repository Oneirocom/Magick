...
const Chat = ({ empty = false }) => {
  ...
  useEffect(() => {
      ...
      axios.get(config.apiUrl + '/spells', {
        ...
        .then(function (response) {
          if (response.data.total == 0) {
            ...
            const temp_tabs = tabs.filter((item) => item.id !== activeTab.id)
            console.log(temp_tabs)
            if (temp_tabs.length === 0) navigate('/home')
            const idx = Math.floor(Math.random() * temp_tabs.length)
            if (temp_tabs.length > 0) navigate(`/chat/${temp_tabs[idx].URI}`)
            dispatch(
              openTab({
    ...
    if (tabs.length > 0 && !activeTab) {
      navigate(`/chat/${tabs[Math.floor(Math.random() * tabs.length)].URI}`)
    }
  ...
  return (
    <>
      {/* TODO: Add Export chat (CSV in OpenAI's fine tunning format). Delete chat.  */}
      <TabBar tabs={tabs} activeTab={activeTab} />
      <TabLayout>
        {!empty && (
          <ChatWindow tabs={tabs} pubSub={pubSub} activeTab={activeTab} />
        )}
      </TabLayout>
    </>
  )
