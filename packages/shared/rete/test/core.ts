...
describe('Core', () => {
  ...
  it('events', () => {
    assert.doesNotThrow(
      () => editor.trigger('nodecreate'),
      Error,
      'nodecreate events exist'
    )
    assert.throws(
      () => editor.on('wrngevent' as any, () => {}),
      Error,
      'throw exception on non-exist event'
    )
    assert.doesNotThrow(
      () => editor.on(['nodecreate'], () => {}),
      Error,
      'on events array'
    )
  })
