...
const info = `Evaluate whether an input number is less than or equal to the value. Triggers true if it is, false if it isn't.`

/**
 * InRange Class represents a component that checks if a given number is within a specified range.
 */
export class LessThanOrEqual extends MagickComponent<void> {
  constructor() {
    super(
      'Less Than Or Equal',
      {
