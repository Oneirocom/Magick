...
/**
 * Insert component responsible for inserting records using any providers
 * available in Magick.
 */
export class Insert extends MagickComponent<Promise<WorkerReturn>> {
  constructor() {
    super(
      'Insert Record',
      {
  ...
  builder(node: MagickNode) {
    ...
    /**
     * Configure the provided node according to the inserted database provider.
     */
    const configureNode = () => {
  ...
  /**
   * Worker for processing insert queries.
  ...
  async worker(
    ...
    // get the provider for the inserted model
    const provider = completionProviders.find(provider =>
      provider.models.includes(model)
    ) as CompletionProvider
