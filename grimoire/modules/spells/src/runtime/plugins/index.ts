...
export default defineNovaPlugin<SpellFeatures, any, any, any, any>({
  ...
  after: (nitro, br) => {
    ...
    console.info("Spell module initialized. Use nitro.spellRegistry to access spells.");
  },
});
  ...
  try {
    ...
    for (const file of files) {
      if (path.extname(file) === '.json') {
        ...
        const spell: Spell = {
          ...serializedSpell,
          graph: readGraphFromJSON({ graphJson: serializedSpell.graph, registry: useRuntimeConfig().registry }),
        };
